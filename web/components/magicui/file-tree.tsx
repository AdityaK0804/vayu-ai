...
type TreeViewProps = {
  ...
  /** Controlled selection (e.g. active dashboard view). */
  selectedId?: string
...
const Tree = forwardRef<HTMLDivElement, TreeViewProps>(
  (
    ...
    useEffect(() => {
      const target = selectedIdProp ?? initialSelectedId
      if (target) {
        expandSpecificTargetedElements(elements, target)
      }
    }, [initialSelectedId, selectedIdProp, elements, expandSpecificTargetedElements])
...
type FolderProps = {
  ...
  isSelectable?: boolean
  isSelect?: boolean
  /** Per-folder icons (override Tree-level openIcon/closeIcon). */
  openIcon?: React.ReactNode
  closeIcon?: React.ReactNode
...
>(
  (
    ...
    const open = openIconProp ?? openIcon
    const closed = closeIconProp ?? closeIcon
    ...
    return (
      ...
      >
        <AccordionPrimitive.Trigger
          ...
          onClick={() => {
            // folders expand/collapse only — selection is for leaf items (nav views)
            handleExpand(value)
          }}
        >
          {expandedItems?.includes(value)
            ? (open ?? <FolderOpenIcon className="size-4" />)
            : (closed ?? <FolderIcon className="size-4" />)}
          <span>{element}</span>
        </AccordionPrimitive.Trigger>
